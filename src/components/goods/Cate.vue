  ...
  <div>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button @click="editCateDialogClosed">重 置</el-button>
  ...
  methods: {
    ...
    showEditCateDialog(id) {
      ...
      this.$http.get('categories/' + id).then(res => {
        ...
        this.editCateForm.cat_name = res.data.data.cat_name
    ...
    modifyCateInfo() {
      this.$refs.editCateFormRef.validate(valid => {
        if (!valid) return
        this.$http.put('categories/' + this.cateId, { cat_name: this.editCateForm.cat_name }).then(res => {
          if (res.data.meta.status !== 200) {
